...
func (s *PostgresStore) UpsertPhysicalGPU(ctx context.Context, gpu PhysicalGPURecord) error {
	_, err := s.pool.Exec(ctx, `
		INSERT INTO physical_gpus (uuid, model, vendor, memory_total_gb, node_id, last_seen_at)
		VALUES ($1, $2, $3, $4, $5, NOW())
		ON CONFLICT (uuid)
		DO UPDATE SET last_seen_at = NOW(), model = EXCLUDED.model, node_id = EXCLUDED.node_id
	`, gpu.UUID, gpu.Model, gpu.Vendor, gpu.MemoryTotalGB, gpu.NodeID)
	if err != nil {
...
func (s *PostgresStore) UpsertComputeResource(ctx context.Context, res ComputeResourceRecord) error {
	_, err := s.pool.Exec(ctx, `
		INSERT INTO compute_resources (resource_uuid, physical_gpu_uuid, resource_type, memory_gb, last_seen_at)
		VALUES ($1, $2, $3, $4, NOW())
		ON CONFLICT (resource_uuid)
		DO UPDATE SET last_seen_at = NOW(), resource_type = EXCLUDED.resource_type
	`, res.ResourceUUID, res.PhysicalUUID, res.ResourceType, res.MemoryGB)
	if err != nil {
...
func (s *PostgresStore) UpsertResourceContext(ctx context.Context, rc ResourceContextRecord) error {
	...
	_, err := s.pool.Exec(ctx, `
		INSERT INTO resource_contexts (context_id, resource_uuid, user_label, hostname, attached_at)
		VALUES ($1, $2, $3, $4, NOW())
		ON CONFLICT (context_id)
		DO UPDATE SET hostname = EXCLUDED.hostname, detached_at = NULL
	`, contextID, rc.ResourceUUID, rc.UserLabel, rc.Hostname)
	if err != nil {
...
// ListGPUs returns a summary of all registered GPUs.
func (s *PostgresStore) ListGPUs(ctx context.Context) ([]GPUSummary, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT cr.resource_uuid, cr.physical_gpu_uuid, pg.model, cr.resource_type, pg.node_id
		FROM compute_resources cr
		JOIN physical_gpus pg ON pg.uuid = cr.physical_gpu_uuid
...
// GetGPU returns details for a single GPU by resource UUID.
func (s *PostgresStore) GetGPU(ctx context.Context, uuid string) (*GPUDetail, error) {
	var g GPUDetail
	err := s.pool.QueryRow(ctx, `
		...
		FROM compute_resources cr
		JOIN physical_gpus pg ON pg.uuid = cr.physical_gpu_uuid
		WHERE cr.resource_uuid = $1
	`, uuid).Scan(&g.ResourceUUID, &g.PhysicalUUID, &g.Model, &g.ResourceType, &g.NodeID,
